...
#### API
- KerasのシーケンシャルAPIはレイヤーの単純な線形スタックには優れています
- 関数型APIは複雑なモデルの構築に不可欠な柔軟性と制御性に優れています。
  例：複数の入出力を持つモデル、 共有レイヤー、さらには非線形のデータフローを持つモデルを作成できます。
- 一番柔軟性があるのが、サブクラス化API  
  例：フォワードパスを静的に定義できないモデルを構築する必要がある場合に特に役立ちます。  
  カスタムトレーニングループや非標準アーキテクチャの研究開発に広く使用されています。  
  動的グラフが使用できる。
...
# 事前学習済みモデル
事前学習済みモデルとは、大規模なデータセット（通常は特定のタスク用）で既に学習済みの機械学習モデルを指す。  
新しいタスクやデータセットでの使用や微調整ですぐに使用可能。  
事前学習済みモデルの基本的な考え方は、学習に使用したデータから既に有用なパターンや特徴を学習しているため、ゼロから構築する必要がない。
## ResNet、VGG（画像分類）
画像分類タスクでよく使用される事前学習済みモデル。  
数百万枚の画像から学習しており、特定の画像関連タスクに合わせて微調整可能。

## BLIP（画像キャプション作成と要約）
BLIPは、画像のキャプションと要約を生成できる事前学習済みモデル。  
画像とテキストのペアで既に学習されているため、新しい画像の説明的なキャプションを簡単に生成可能。

# 評価指標
| 　 | 予測：Positive | 予測：Negative |
|-----|------|-----|
| 実際：Positive | 真陽性（TP) | 偽陰性（FN) |
| 実際：Negative | 偽陽性（FP) | 真陰性（TN) |

## 正解率
全データの中で、どれだけ予測が当たったかの割合  
真陽性のデータ数（TP)＋真陰性のデータ数（TN)  
/全データ数（TP＋FN＋FP＋TN)
## 適合率（Precision）
予測が正の中で、実際に正であったものの割合  
真陽性のデータ数（TP)  
/真陽性のデータ数（TP) ＋偽陽性のデータ数(FP)
## 再現率（Recall, Sensitivity）
実際に正であったものの中で、正と予測できた割合  
真陽性のデータ数（TP)  
/真陽性のデータ数（TP) ＋偽陰性のデータ数(FN)
## F値
適合率と再現率の調和平均。適合率のみで判断すると、予測が偏っているときも値が高くなるので、F値を用いることも多い  
２ｘ適合率x再現率  
/適合率＋再現率
