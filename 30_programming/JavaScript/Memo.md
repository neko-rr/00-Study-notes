...
関数名(値);
```
で、出力で「実行処理」の内容()の中に、引数を指定していた場合は、「値」を入れる  
...
```
## 戻り値のある関数：return
関数はreturnの値を戻り値として返す
- returnは、戻り値を返すだけでなく、関数の処理を終了させる性質も持っています。よって、returnの後にある関数内の処理は実行されませんので注意
```JavaScript
const 関数名 = function(a, b) {
  return a + b;
};
```
## スコープ
- 定数や変数の使用できる範囲のことをスコープと呼びます。スコープは、図のように定数や変数を定義した場所によって異なる
