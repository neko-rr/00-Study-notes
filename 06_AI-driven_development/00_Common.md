...
5. チームでレビューする際のポイント
重複ストーリーがないか確認する
→ 同じ目的のストーリーが複数ある場合は統合。
ユーザー視点が維持されているか
→ 「開発者がやりたいこと」ではなく「利用者がしたいこと」。
Claude Codeでチェック
→「このユーザーストーリーに抜け漏れがあるか確認して」と依頼。

6. Claude Codeで点検を繰り返す流れ
Claude Codeで初稿を作成
チームレビューを実施（Notionでコメント）
Claude Codeに修正版を依頼
Claude Codeで最終確認（再点検）
Claude Codeは、反復プロンプトに強いAIです。
「もう一度点検して」と繰り返すほど、品質が高まります。


手順3：コメントを削除・統合
チームレビューでつけたコメントを反映し、不要なコメントを削除。
修正内容は「更新履歴」欄に残しておくとよいでしょう。
📝 更新履歴
- 2025/10/15：Claude Codeの指摘を反映（通知条件を追加）
- 2025/10/12：非機能要件のパフォーマンス条件を

5. チーム共有とアーカイブ
仕上がった要件定義書は、チーム全体で共有しましょう。
おすすめの運用方法は以下の通りです。
Notion：メイン管理＋コメント議論
GitHub / Google Drive：履歴バックアップ
Claude Code：将来の改訂時に再レビュー依頼
Claude Codeのプロジェクト履歴機能を使えば、
「v1.0（初版）」「v1.1（修正版）」の比較も自動化できます。
...
## 1. システム概要
- システム全体像と構成図
- 主要技術（Next.js / Supabase / Vercel など）

## 2. 画面設計
- 画面一覧（画面ID・名称）
- 画面遷移図（Figma or Markdown）
- UI要素（入力欄・ボタン・状態表示など）
...
データの関係性と構造を明確にします。
Claude Codeへの依頼例：
「この要件定義をもとに、ER図のMarkdownを出力してください。」
Claude Code出力例：
Claude.md
User ──< Task ──< Progress
④ API設計
バックエンドとフロントエンドをつなぐ仕様です。
テンプレート例：
エンドポイント
メソッド
概要
...
このユニットでは、Figmaを使って最短で画面ラフを作る方法を学びました。
Figmaで「構造」だけを描く（色や装飾は不要）
Claude Codeと連携すれば、画面設計書を自動生成可能
コメント・プロトタイプ機能で操作の流れを可視化

セキュリティの基本
ソフトウェア開発におけるセキュリティは、大きく以下の観点で考えられます。
機密性（Confidentiality）
→ 機密情報（APIキー、個人情報など）を外部に漏らさない
完全性（Integrity）
→ データが改ざんされず正しい状態で保持される
可用性（Availability）
→ サービスが安定して利用できる状態を維持する
この3つは CIAトライアングル と呼ばれ、セキュリティ設計の基本原則となります。

具体例：リスクシナリオ
ケース1：Claudeに「認証処理のコードを書いて」と依頼 → 平文でパスワード保存するコードが提案される
ケース2：依頼時に誤って本番用のAPIキーを入力してしまう → 外部送信される可能性
ケース3：古いライブラリを推奨され、そのまま利用 → 既知の脆弱性を抱えたままリリース

Claude Codeにコードを依頼する際は、process.env.API_KEY のように変数として指定
