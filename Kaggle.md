...
タイムゾーンの記載はUTC（協定世界時）の場合が多いです．UTCから日本時間（JST）への変換には９時間を加える必要があります．例えば，UTCで24時が期限と書かれている場合は，日本時間では翌日の９時が期限となります．

　右上の「Copy and Edit」をクリックすると，編集画面に遷移します．この時点ですでに元のNotebookとは別物なので，自分の好き勝手に編集して問題ありません．

Kaggleでは，次の方法で自分が作成した機械学習モデルの予測結果をsubmitできます．
￼KaggleのNotebook経由
...
Kagglerの間で「Trust CV」という言葉があります．「Public LBのスコアよりも，自分で計算したCVスコアを信じよう」という意味なのですが，こういう言葉が存在するくらい，大切な話題ですね．
bestfittingさんは最終的な２つのsubmitに，「安全な」モデルと比較的「危険な」モデルを選ぶと話していますね．u++さんはどういう方針ですか？
...
チームマージの効果は，モデルの多様性を獲得して，アンサンブルでのスコアの向上のためということもありますよね．金メダル手前の者同士が組んでアンサンブルして金メダルに到達したり，金メダル圏内の人同士で組んで賞金圏内に入ることも良くあります．モデルの多様性ということだと，チームマージ期限の直前にチームマージするのが一番良いのですが，いつチームマージするかは難しいですよね．
