...
- Kaggleの無料GPUの節約になる
- 待機時間の節約になる
- 方法は？多分、下記のセルの内容をファイルに書き出す（ファイルがない場合は生成します）  
⇒かなり時間の節約になっているが、学習済みモデルのコードをコピペして、推論だけ編集するのは、Kaggle的にOKなのだろうか？600人も同じスコアを出しているのだから、それが普通？
### セルの内容をファイルに書き出す（ファイルがない場合は生成します）
- データセットやハイパーパラメータを別のファイルに書き出す。
- 書き出したファイルをどのコードに持って行っても動くようにセル内で、必要なimportをもう一度行う事
```Python
...
## outputにファイルが無い事になっている
ファイルを開いた状態で全てを実行した時は、提出用ファイルが作成されているが、「submit」を押すと、ファイルが存在しないと提出できない。  
「save&run all」をすると必ず途中で止まるのが原因。  
⇒都度実行ではなくて、通しで実行できるようにコードを修正する。
